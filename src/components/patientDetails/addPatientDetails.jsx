...
class AddPatientPersonalDetails extends React.Component {
  ...
  validate = () => {
    const errors = {};
    const result = Joi.validate(this.state.patientPersonalDetails, this.schema , { 
      abortEarly: false,});
    ...
    if (this.state.errors) return;
        ...
        axios.post("http://localhost:8080/PatientPersonalDetails/add",this.state.patientPersonalDetails)
        .then((res) => {
            console.log(res.data);
            alert("added patient " + this.state.patientPersonalDetails.pName + "successfully!");
